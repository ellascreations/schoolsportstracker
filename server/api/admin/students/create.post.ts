...
export default defineEventHandler(async (event) => {
  ...
  const { data: created, error } = await admin.auth.admin.createUser({ email, password, email_confirm: true, user_metadata: { first_name: firstName, last_name: lastName, role: 'student', school_id: profile.school_id } })
  ...
  const { error: profileError } = await admin.from('profiles').update({
    ...
    year_level: body.year_level || null, house_id: body.house_id || null, school_id: profile.school_id, role: 'student', active: true, updated_at: new Date().toISOString()
