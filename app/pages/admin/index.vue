...
<template>
  <main class="mx-auto max-w-7xl px-4 py-8">
    ...
    <p v-if="profile?.school" class="mt-2 text-slate-500">{{ profile.school.name }}</p>

    <div class="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <NuxtLink v-for="item in items" :key="item[0]" :to="item[2]" class="rounded-xl border bg-white p-6 shadow-sm hover:border-violet-300">
